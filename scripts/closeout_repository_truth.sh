...
scheduled_report_is_acceptable() {
  ...
  : "${MAILGLASS_JQ:?MAILGLASS_JQ is required}"
...
"$MAILGLASS_GIT" -C "$repo" check-ignore -q -- "$output_rel" || usage
"$MAILGLASS_GIT" -C "$repo" check-ignore -q -- "$components_rel" || usage
mkdir -m 700 "$components_dir"

stable_porcelain() {
  "$MAILGLASS_GIT" -C "$repo" status --porcelain=v1 --untracked-files=all 2>/dev/null || printf 'git_status_failed\n'
}

component() {
  local name="$1" status="$2" reason="$3" source="$4" component_tmp
  component_tmp=$(mktemp "$components_dir/$name.XXXXXX")
  "$MAILGLASS_JQ" -n --arg status "$status" --arg reason "$reason" --arg source "$source" \
  ...
  if [ -n "$(stable_porcelain)" ]; then
    component_tmp=$(mktemp "$components_dir/$name.XXXXXX")
    "$MAILGLASS_JQ" -n --arg source "$source" '{status: "blocked", reason: "post_write_porcelain_dirty", source: $source}' >"$component_tmp"
    mv "$component_tmp" "$components_dir/$name.json"
  fi
}
...
set +e
(cd "$repo" && "$MAILGLASS_MIX" mailglass.repo.hygiene --check --format json) >"$hygiene_raw" 2>&1
hygiene_exit=$?
set -e
if "$MAILGLASS_JQ" -e '
  type == "object" and
...
' "$hygiene_raw" >/dev/null 2>&1; then
  hygiene_status=$("$MAILGLASS_JQ" -r '.status' "$hygiene_raw")
  if [ "$hygiene_status" = pass ] && [ "$hygiene_exit" -ne 0 ]; then component hygiene cannot-check command_status_mismatch "$hygiene_raw"; else component hygiene "$hygiene_status" "$("$MAILGLASS_JQ" -r '.reason' "$hygiene_raw")" "$hygiene_raw"; fi
...
[ -z "$final_porcelain" ] && [ "$("$MAILGLASS_JQ" -r '.status' "$output")" = pass ]
}
