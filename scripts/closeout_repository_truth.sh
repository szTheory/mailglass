...
component() {
  ...
  if [ -n "$(stable_porcelain)" ]; then
    jq -n --arg source "$source" '{status: "blocked", reason: "post_write_porcelain_dirty", source: $source}' >"$components_dir/$name.tmp"
    mv "$components_dir/$name.tmp" "$components_dir/$name.json"
  fi
}
...
if (cd "$repo" && bash scripts/scheduled_control_evidence.sh sweep --output "$scheduled_raw") >/dev/null 2>&1 && jq -e --arg sha "$head_sha" 'type == "object" and .expected_main_sha == $sha and (.status == "pass" or .status == "blocked") and .evidence_valid == true and (.controls | type == "array" and length > 0) and all(.controls[]; .evidence_valid == true and .source_run.event == "schedule" and .source_run.status == "completed" and .source_run.head_branch == "main" and .source_run.head_sha == $sha and .result.workflow_sha == $sha and (.source_run.updated_at | fromdateiso8601) >= (now - 10800) and (.result.status == "pass" or (.result.status == "blocked" and (.result.payload_sha256 | type == "string" and length > 0))))' "$scheduled_raw" >/dev/null 2>&1; then component scheduled pass current_provenance_valid "$scheduled_raw"; elif jq -e 'type == "object" and (.status == "pending" or .status == "cannot-check")' "$scheduled_raw" >/dev/null 2>&1; then component scheduled "$(jq -r '.status' "$scheduled_raw")" "$(jq -r '.reason // "scheduled_evidence_incomplete"' "$scheduled_raw")" "$scheduled_raw"; else component scheduled cannot-check malformed_stale_or_mismatched_scheduled_evidence "$scheduled_raw"; fi
