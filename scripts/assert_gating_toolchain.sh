...
# Erlang is compared exactly. `.tool-versions`, setup-beam's strict version-file
# mode, and both repository Dockerfiles share the same supported patch.
...
expected_otp="$(awk '$1 == "erlang" { print $2 }' "$tool_versions")"

if [ -z "$expected_elixir" ] || [ -z "$expected_otp" ]; then
...
actual_otp="$(erl -noshell -eval '{ok, V} = file:read_file(filename:join([code:root_dir(), "releases", erlang:system_info(otp_release), "OTP_VERSION"])), io:format("~s", [string:trim(binary_to_list(V))]), halt().')"

if [ "$actual_elixir" != "$expected_elixir" ] || [ "$actual_otp" != "$expected_otp" ]; then
  cat >&2 <<EOF
assert_gating_toolchain: this container is NOT the gating toolchain.

  declared by ${tool_versions} : Elixir ${expected_elixir} / OTP ${expected_otp}
  actually running here        : Elixir ${actual_elixir} / OTP ${actual_otp}
...
echo "gating toolchain confirmed: Elixir ${actual_elixir} / OTP ${actual_otp}"
