...
- [ ] **PIN-01**: Both sibling `{:mailglass, "== X.Y.Z"}` pins are replaced with `~>` — inbound
  `{:mailglass, "~> 1.10 and >= 1.10.2"}`, admin `{:mailglass, "~> 1.10"}` — behind the existing
  `MIX_PUBLISH` gate (LD-2).
- [ ] **PIN-02**: `stability_contract_test` and `publish.check` (`verify_deps`/`verify_linked_constraint`)
...
- [ ] **PIN-03**: The two `== X.Y.Z` sed rewrites are deleted from `release-please.yml`; a core patch
  release touches zero sibling pin lines (LD-3).
- [ ] **PIN-04**: The pin change ships a CHANGELOG entry and documents `mix hex.retire` as the rollback
  lever that replaces the `==` wall (LD-5).
- [ ] **PIN-05**: The whole pin change lands as one indivisible change verified green on a bare main
