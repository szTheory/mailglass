...
> Per-phase validation contract for feedback sampling during execution. The contract covers every executor task in Plans 164-01 through 164-07; Wave 0 implementation remains pending until the named tests are created and run.
...
| **Quick run command** | `mix test test/scripts/phase_164_repository_truth_test.exs test/scripts/phase_164_closeout_test.exs test/mailglass/publish/maintaining_release_gate_contract_test.exs test/mailglass/docs_contract_test.exs --warnings-as-errors --no-deps-check` |
...
- **After every plan wave:** Run `mix ci.fast` after all implementation tasks in the wave are integrated; Wave 4 is an external-state checkpoint whose focused contract command runs before handoff.
- **Before `$gsd-verify-work`:** Run `mix ci.fast`, then execute Plan 164-07's report gate against exact protected `main` and fresh scheduled-control evidence.
...
- [x] All eight executor tasks across all seven plans have an automated verification row.
...
- [x] TRTH-01, TRTH-02, and TRTH-03 each have task-level coverage through their terminal exact-main verification.
