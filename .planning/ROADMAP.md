...
- 🚧 **v2.1 Postgres + Admin URL Hardening** - Phases 138-140 (**ACTIVE** - opened 2026-07-07)

## Phases (Active - v2.1 Postgres + Admin URL Hardening)
...
cross-cutting verification and artifact reconciliation.

**Research flags:** none blocking. Milestone-level decisions are settled from the 2026-07-07 research pass;
per-phase planning should inspect only the relevant code edges before implementation.

## Phase Details (Active - v2.1)

### Phase 138: Schema-prefix no-search-path hardening

**Goal:** Ensure runtime reads/writes that touch mailglass tables still target the configured schema when
...
**Plans:** TBD by `/gsd-plan-phase 138`

**Success Criteria** (what must be TRUE):

1. `Mailglass.Webhook.Replay` projection updates are fixed/proven under hostile `search_path`.
...
**Goal:** Prove admin CSS/font asset URLs are mount-aware on first HTML load and hard refresh, including
...
**UI hint:** yes - verification-only; no visual redesign, token change, component change, or screenshot
baseline work.
...
*Last updated: 2026-07-07 - v2.1 roadmap defined.*
