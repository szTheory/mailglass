...
- [ ] 45-10-PLAN.md — Gap Round 2, Wave 1 (DEFINING GOAL): register the inert `StreamPolicyConsistent` guard in `.credo.exs` (keep `mix credo --strict` green) + extend the meta-test to assert `.credo.exs` registration for every check (close the registration blind spot, with teeth) + fix the false GenSmtp-key comment (WR-04) + de-dupe `integration_test.exs` against the live config (WR-05) [TELE-06]
- [ ] 45-11-PLAN.md — Gap Round 2, Wave 1 (parallel): WR-02 charter — extend `TelemetryEventConvention` to validate literal event names at the package's own `*_span` helper call sites (real inbound coverage) + fixtures (fire/refute) + real-inbound proof against `mailglass_inbound` telemetry.ex + precise moduledoc [TELE-06]
- [ ] 45-12-PLAN.md — Gap Round 2, Wave 1 (parallel): egress PII hardening — broaden `NoPiiInResponseBody` to catch error-var/bare-variable leaks (keep static-body + Jason-encoded-body carve-outs) + documented intra-function limitation + `err`-bound regression test + IN-01 gen_smtp `available?/0` doc note [TELE-06]
