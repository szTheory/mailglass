...
**v0.3.2 shipped 2026-04-29. Milestone v0.4 "Operator Confidence" is active as of 2026-05-01. Phase 22 completed the operator data foundation; Phase 23 is next.**
...
- Preview LiveView shipped at v0.1; prod admin operator hardening is the active v0.4 milestone
...
## Current Milestone: v0.4 Operator Confidence
...
- Webhook replay with audit context and tenant-safe boundaries
- `mix mail.doctor` core DNS checks: SPF, DKIM, DMARC, MX, BIMI
- Runtime per-tenant adapter resolver
- Release/install closure for `Issue #25` and `Issue #9`

**Why now:** Provider coverage is complete. The next product gap is not transport breadth; it is whether an adopter can inspect, diagnose, and safely operate transactional mail from inside a real Phoenix app.

## Next Milestone Goals (post-v0.4)

- **v0.5 Adoption Hardening** — `mix mailglass.gen.mailable`, richer test assertion helpers, webhook troubleshooting/runbooks, per-domain rate limiting, and broader install/upgrade polish once the operator story is credible.
- **v1.0+ Inbound** — `mailglass_inbound` separate sibling package (Action Mailbox equivalent: Router DSL, Mailbox behaviour, ingress plugs for Postmark/SendGrid/Mailgun/SES, SMTP relay via `gen_smtp`, async routing via Oban, dev Conductor LiveView) after the outbound/operator core is stable.
...
- [ ] `ADMIN-01` / `ADMIN-05` — production-safe admin mount and step-up auth for destructive operator actions
- [x] `ADMIN-02` / `ADMIN-03` / `ADMIN-04` — operator data foundation completed in Phase 22
...
---
*Last updated: 2026-05-01 — activated v0.4 Operator Confidence and aligned active requirements with completed Phase 22 groundwork.*
