...
| D-30 | The operator/inbound admin UI shows the **full** recipient and sender email address on every surface; `Components.mask_recipient/1` is retired for `recipient_display/1` (raw, `nil`→"Unavailable") | An authenticated, tenant-scoped operator inside the admin UI is already privileged to see the address they support; presentation-layer masking against shoulder-surfing is over-the-top for this audience and adds friction to real support work. **Narrows/supersedes the D-33-17/18 UI-display posture only; D-33-15 telemetry/log/doc no-PII rule is UNCHANGED** (not a security-boundary change) | ✓ Adopted 2026-07-18 — one audited `recipient_display/1`, whole-UI unmask; see `.planning/milestones/v0.6-phases/33-observability-incident-support/33-RESEARCH.md` supersede note |
