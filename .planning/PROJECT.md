...
- **MJML as a default rendering path** — HEEx + Phoenix.Component with MSO fallbacks IS the default. MJML stays as an opt-in `Mailglass.TemplateEngine.MJML` adapter (via [mrml](https://hex.pm/packages/mrml) NIF, no Node) for teams who insist on it. The killer differentiator is *not needing* MJML.
...
- **Optional deps** (with `optional: true` in mix.exs and `Code.ensure_loaded?/1` guards): `:oban`, `:opentelemetry`, `:sigra`, `:mrml`, `:gen_smtp`. CI must pass `mix compile --no-optional-deps --warnings-as-errors`.
...
| D-18 | Renderer default is HEEx + `Phoenix.Component` with MSO VML fallbacks; MJML is opt-in `Mailglass.TemplateEngine.MJML` via mrml NIF | Native composition, no Node, killer differentiator vs React Email + Mailing | — Pending |
