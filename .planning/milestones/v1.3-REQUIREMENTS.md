# Requirements Archive: v1.3 Adopter Trust Proof
...
- [x] **HOST-01**: Adopter can boot one maintained Phoenix reference host app from a clean checkout using documented setup and published package constraints.
- [x] **HOST-02**: Reference host app integrates through documented public Mailglass seams only, with no copied provider internals.
- [x] **HOST-03**: Reference host app includes an explicit proof-scope allowlist and non-goals so it does not become a second product.

### Deterministic Trust Journey

- [x] **JOUR-01**: One deterministic trust runner command proves install -> preview -> send -> webhook ingest -> operator troubleshooting.
- [x] **JOUR-02**: Trust fixtures use stable IDs/payloads so local and CI assertions are reproducible.
...
- [x] **EVID-01**: CI has a required repo-head trust lane that fails on missing journey checkpoints.
...
- [x] **EVID-04**: Trust lanes emit machine-readable checkpoint artifacts used as release evidence.
...
|-------------|-------|--------|
| HOST-01 | Phase 52 | Complete |
| HOST-02 | Phase 52 | Complete |
| HOST-03 | Phase 52 | Complete |
| JOUR-01 | Phase 57 | Complete |
| JOUR-02 | Phase 57 | Complete |
...
*Last updated: 2026-05-31 after v1.3 milestone gap closure verification*
