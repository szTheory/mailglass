# Requirements: mailglass v0.1 (ARCHIVED)

**Status:** ✅ SHIPPED 2026-04-26 (v0.1.0 + v0.1.1 on Hex.pm)
**Defined:** 2026-04-21
**Archived:** 2026-04-26
**Core Value:** Email you can see, audit, and trust before it ships.

> **This is the archived v0.1 milestone requirements snapshot.** All 84 v1 REQ-IDs satisfied. The v2 (= v0.5) and Out-of-Scope sections below are tracked here for historical reference; the next milestone re-derives its own requirements via `/gsd-new-milestone`.
>
...
- [x] **PREV-01**: `mailglass_admin` is a separate Hex package. Its `mix.exs` declares `{:mailglass, "== <pinned_version>"}` — sibling versions never drift. (D-01, DIST-01 prevention)
- [x] **PREV-02**: `MailglassAdmin.Router` exposes a macro `mailglass_admin_routes(path, opts)` for adopter routers. Mount path is the adopter's first arg with no default. (PHX-02 prevention)
- [x] **PREV-03**: `MailglassAdmin.PreviewLive` (dev-only mount) renders: a sidebar listing all `Mailglass.Mailable` modules + their preview functions auto-discovered via `preview_props/0` callback; HTML/Text/Raw/Headers tabs in main pane; device width toggle (mobile/tablet/desktop); dark/light mode toggle; live-assigns form for each `preview_props/0` field. (TS-13, DF-01, DF-10)
- [x] **PREV-04**: Live-reload via Phoenix LiveReload integration: editing the mailable source file refreshes the preview without page reload.
- [x] **PREV-05**: `MailglassAdmin.Components` ships responsive, mobile-first UI components matching the brand book palette (Ink/Glass/Ice/Mist/Paper/Slate, Inter + IBM Plex Mono). daisyUI 5 + Tailwind v4 (Phoenix 1.8 default; no Node required for adopters' build). (BRAND-01)
- [x] **PREV-06**: `mailglass_admin/priv/static/` is a committed compiled bundle. CI runs `git diff --exit-code` after `mix mailglass_admin.assets.build`. Hex tarball size <2MB. (PHX-03, DIST-02 prevention)
...
- [x] **INST-02**: Installer is idempotent. Reruns on a host with prior mailglass install detect existing files and write `.mailglass_conflict_*` sidecars rather than clobbering. Second-rerun integration test asserts zero file modifications. (DIST-03 prevention)
...
- [x] **INST-04**: `mix verify.phase<NN>` aliases per phase per the engineering DNA convention. One focused mix task per concern, never a kitchen-sink verifier.

### CI/CD

- [x] **CI-01**: GitHub Actions workflows: `ci.yml` (PR + main push: format, compile w/ warnings-as-errors, compile `--no-optional-deps --warnings-as-errors`, ExUnit, Credo `--strict`, Dialyzer w/ cached PLT, `mix docs --warnings-as-errors`, `mix hex.audit`), `dependency-review.yml`, `actionlint.yml`, `release-please.yml`, `publish-hex.yml` (publish only from protected ref + GitHub Environment with required reviewers). (TS-20)
- [x] **CI-02**: Required test matrix: 1 cell (Elixir 1.18 / OTP 27, current stable). Wider matrix runs nightly cron + `workflow_dispatch`, advisory only.
- [x] **CI-03**: Conventional Commits enforced via PR title check. Squash-merge workflow keeps casual contributor UX low-friction.
- [x] **CI-04**: Release Please configured with manifest + linked-versions plugin. `mailglass` and `mailglass_admin` ship coordinated releases; `mailglass_admin/mix.exs` declares `{:mailglass, "== <new-version>"}`. (D-16, DIST-01 prevention)
- [x] **CI-05**: Hex tarball contents whitelisted to `lib/`, `priv/templates/`, `mix.exs`, `LICENSE`, `README.md`, `CHANGELOG.md`. Tarball size <500KB for `mailglass`, <2MB for `mailglass_admin`. (PHX-03 prevention)
- [x] **CI-06**: All third-party GitHub Actions pinned to commit SHA. Dependabot watches both `mix.lock` and `.github/workflows/`.
- [x] **CI-07**: `HEX_API_KEY` is a GitHub Environment secret with required reviewer approval. PR jobs never see it. (DIST-05, CI-03 prevention)

### Documentation

- [x] **DOCS-01**: ExDoc configured with `main: "getting-started"`, `source_url`, `homepage_url`, source-version refs, full extras + module groups. ExDoc 0.40+ ships `llms.txt` automatically.
- [x] **DOCS-02**: Guides set: `getting-started.md`, `authoring-mailables.md`, `components.md`, `preview.md`, `webhooks.md`, `multi-tenancy.md`, `telemetry.md`, `testing.md`, `migration-from-swoosh.md`. Every guide ends with a runnable end-to-end example.
- [x] **DOCS-03**: Migration guide from raw Swoosh + `Phoenix.Swoosh` is testable: an adopter following it in a single afternoon reaches mailglass parity with no production behavior change. (TS-17)
- [x] **DOCS-04**: Doc-contract tests: every README "Quick Start" snippet compiles; guide setup steps reference real mix tasks; config examples validate against NimbleOptions; admin nav entries match real LiveView routes. CI gate. (TEST-04 prevention)
- [x] **DOCS-05**: `MAINTAINING.md`, `CONTRIBUTING.md`, `SECURITY.md`, `CODE_OF_CONDUCT.md` ship in the repo root. SECURITY.md documents responsible disclosure for webhook signature bypass + tenant isolation issues.

### Brand & Voice

- [x] **BRAND-01**: All `mailglass_admin` UI conforms to `prompts/mailglass-brand-book.md`: Ink/Glass/Ice/Mist/Paper/Slate palette, Inter + Inter Tight + IBM Plex Mono typography, mobile-first responsive, no glassmorphism / lens flares / literal broken glass. WCAG AA contrast verified.
- [x] **BRAND-02**: All error messages and log messages match the brand voice ("clear, exact, confident not cocky, warm not cute, modern not trendy, technical not intimidating"). Errors are specific ("Delivery blocked: recipient is on the suppression list" — never "Oops!").
- [x] **BRAND-03**: Documentation prefers the direct word ("preview" over "experience the full rendering lifecycle"). Doc copy reviewed against brand book before publishing.
