...
1. **Public router API shape**
   - Decision: Phase 23 should add a new operator-only public mount surface and preserve `mailglass_admin_routes/2` as the preview-compatible mount. That keeps preview adopters stable while letting production operator routes declare a distinct `live_session`, session whitelist, and auth hook chain. [VERIFIED: mailglass_admin/lib/mailglass_admin/router.ex][VERIFIED: user prompt]
   - Why: it is the smallest API change that satisfies `ADMIN-01` without forcing preview and operator concerns back into one shared macro contract. [CITED: https://hexdocs.pm/phoenix_live_view/Phoenix.LiveView.Router.html]

2. **Normalized actor metadata shape**
   - Decision: Phase 23 should standardize on a minimal actor map contract for future destructive actions:
     - `subject_id` — stable adopter-owned actor identifier
     - `tenant_id` — optional tenant/scope identifier
     - `auth_method` — optional string/atom describing the adopter auth path
     - `recent_auth_at` — optional UTC timestamp for recent-auth freshness
   - Why: this is enough for Phase 24 replay audit context without teaching `mailglass_admin` about adopter user structs or persistence schemas. [VERIFIED: .planning/ROADMAP.md][VERIFIED: user prompt]
