...
#### CR-01 / CR-02: Routes-only durable bindings and legacy-job recovery

**Files modified:** `mailglass_inbound/lib/mailglass_inbound/execution.ex`, `mailglass_inbound/lib/mailglass_inbound/execution/worker.ex`, and focused async, persistence, and worker tests.
**Commit:** `7a453873`
**Applied fix:** Accepted mailbox-only route evidence generated by lower-level `routes:` integrations and resolved it from the finite loaded module set only after verifying the `MailglassInbound.Mailbox` behaviour and `process/1` callback. Durable Oban jobs now run after router-registry restart while rejecting mailbox selector tampering. Pre-binding jobs return the distinct `:legacy_route_binding_missing` outcome; the worker logs a PII-free tenant-scoped replay recovery message and cancels permanently instead of consuming retries.
