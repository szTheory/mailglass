defmodule Mailglass.Events do
  ...
  `mailglass_events`.  `NoRawEventInsert` Credo check enforces
  ...
  # : auto-capture tenant_id + trace_id + occurred_at defaults.
  ...
  # 's OpenTelemetry gateway probes `:otel_tracer`; here we stay
  # conservative and just return nil until  wires explicit trace
  ...
  # Schema prefix threaded here via Keyword.put_new so an explicit caller :prefix wins.
  # This single chokepoint covers all four write sites: append_multi/3 map form,
  # append_multi/3 fn form, do_insert/1 idempotent clause, and do_insert/1 plain clause.
