defmodule Mailglass.Webhook.Reconciler do
  ...
  The entire module body is conditionally compiled behind
  `if Code.ensure_loaded?(Oban.Worker) do ... end`. When Oban is absent,
  the module still defines `available?/0` (returning `false`) so callers
  can probe without depending on `Code.ensure_loaded?/1` at the call
  site. `Mailglass.Application` emits a one-time `Logger.warning` (D-20)
  at boot when Oban is missing, directing operators to either run
  `mix mailglass.reconcile` + `mix mailglass.webhooks.prune` from their
  own cron infrastructure or add Oban to their deps.
  ...
  if Code.ensure_loaded?(Oban.Worker) do
    ...
    Exposed as a public function so `mix mailglass.reconcile` (the
    Oban-absent fallback) can invoke the same code path; also useful in
    tests and for ops engineers who want to run a sweep out-of-band
