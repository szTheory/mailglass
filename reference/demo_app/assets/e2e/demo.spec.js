...
test.describe("mailglass demo evidence", () => {
  ...
  test("dashboard links to preview and operator surfaces", async ({ page }) => {
    await page.goto("/");
    await expect(page.getByRole("heading", { name: /email you can inspect/i })).toBeVisible();
    await expect(page.getByText("Northstar Ops")).toBeVisible();
    await expect(page.getByText("Deliveries", { exact: true })).toBeVisible();

    await page.getByRole("link", { name: /mailables/i }).click();
  ...
  test("inbound operator opens with seeded support mailbox evidence", async ({ page }) => {
    await page.goto("/demo/login?return_to=/ops/mail/inbound?tenant_id=northstar");
