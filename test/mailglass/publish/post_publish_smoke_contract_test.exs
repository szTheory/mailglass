defmodule Mailglass.Publish.PostPublishSmokeContractTest do
  ...
  test "consumer smoke resolves all exact packages from an immutable completed target" do
    ...
    assert workflow =~ "Install deps for policy"
    assert workflow =~ "Compile policy runtime"
    assert workflow =~ "--no-compile --no-deps-check"
    assert workflow =~ "INPUT_CORE"
    assert workflow =~ "[ -n \"$INPUT_CORE\" ] && [ \"$INPUT_CORE\" = \"$core\" ]"
    refute workflow =~ "cat \"$resolved\" >> \"$GITHUB_OUTPUT\""
