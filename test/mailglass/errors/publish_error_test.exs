defmodule Mailglass.PublishErrorTest do
  ...
  describe "new/2 with :publish_blocked_golden_drift" do
    test "includes the exact remediation command in the message" do
      ...
      assert err.message =~ "MIX_INSTALLER_ACCEPT_GOLDEN=1 mix test test/mailglass/install/install_golden_test.exs --warnings-as-errors"
    end

    test "appends subprocess output when present in context" do
      err = PublishError.new(:publish_blocked_golden_drift, context: %{output: "1 test, 1 failure\n\nmismatch snapshot"})
