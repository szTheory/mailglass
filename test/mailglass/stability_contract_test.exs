defmodule Mailglass.StabilityContractTest do
  ...
  describe "stability proof wiring" do
    ...
    test "release automation and publish proof keep mailglass_inbound in sibling-package truth" do
      ...
      # Phase 044.5 Plan 01 bumped the inbound's pinned mailglass dep to
      # `== 1.0.0` (the linked-version target for the v1.0 sibling-group
      # publish). The inbound package itself is at 0.1.0 (first publish).
      assert inbound_mix =~ "{:mailglass, \"== 1.0.0\"}"
      assert inbound_mix =~ "@version \"0.1.0\""
      ...
      # `.planning/publish/mailglass_inbound-publish-summary.json` still pins
      # the pre-v1.0 inbound version (0.3.2). This is intentionally lagging:
      # the file is regenerated by Phase 05 when the v1.1 sibling-group
      # publish completes and the inbound package transitions to 1.x. Until
      # then the assertion stays on 0.3.2 to reflect on-disk truth.
