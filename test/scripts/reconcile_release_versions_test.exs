defmodule Mailglass.Scripts.ReconcileReleaseVersionsTest do
  ...
  test "repository metadata preserves published baselines or an exact generated proposal" do
    ...
    # These were pinned to a literal 2.4.1 while the core and admin summaries
    # lagged: the 2.5.0 publish fan-out advanced only mailglass_inbound's
    # summary, so 2.4.1 was genuinely what those two files said. Both have now
    # been regenerated with `mix mailglass.publish.check` under the Publish
    # Summary Snapshot Protocol (MAINTAINING.md), so all three agree with the
    # published baseline again and the literal is gone.
    #
