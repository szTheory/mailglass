defmodule Mailglass.GeneratedHost.NegativeControlsTest do
  ...
  test "queue and schema controls are closed, isolated, and prove no effects" do
    ...
    assert journey =~ "Supervisor.terminate_child(GeneratedHost.Supervisor, Oban)"
    assert journey =~ "Supervisor.restart_child(GeneratedHost.Supervisor, Oban)"
    refute journey =~ ":unavailable_adapter"
